...
contract QuadrupleScript is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        (address hookAddress, bytes32 salt) =
            HookMiner.find(CREATE2_DEPLOYER, permissions, type(Quadruple).creationCode, abi.encode(address(manager)));

        // ----------------------------- //
        // Deploy the hook using CREATE2 //
        // ----------------------------- //
        vm.broadcast();
        Quadruple quadruple = new Quadruple{salt: salt}(manager);
        require(address(quadruple) == hookAddress, "QuadrupleScript: hook address mismatch");
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(quadruple), posm, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
